 ...
 * 2014-8-1上午11:10:04
 */
public class SnfncPositiveSupply0051ResponseInterceptor implements Interceptable<SnfncPositiveResponseMessage> {
    ...
    public SnfncPositiveResponseMessage intercept(SnfncPositiveResponseMessage message) throws Exception {       
        //响应报文添加币种表示
        if (message != null) {
            String currency = this.snfncPositivePersistence.query0051Currency(message);
            String accountNo = this.snfncPositivePersistence.query0051AccountNo(message);
            String queryDate = this.snfncPositivePersistence.query0051QueryDate(message);  //yyyy-MM-dd HH:mm:ss
